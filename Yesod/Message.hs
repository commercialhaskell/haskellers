...
import Yesod (YesodMessage (..), toHtml, preEscapedString)
...
mkMessage dt folder lang = do
    ...
    return
        ...
        , InstanceD
            []
            (ConT ''YesodMessage `AppT` VarT sub `AppT` dt')
            [ TySynInstD ''Message [VarT sub, dt'] (ConT mname)
...
toClauses dt (lang, defs) =
  ...
  where
    go def = do
        ...
        return $ Clause
            [WildP, WildP, ConP (mkName ":") [VarP a, WildP], pat]
            (GuardedB [(guard, bod)])
            []
...
mkBody dt cs vs ct = do
    ...
    return (pat, e)
  where
    toH pes _ (Raw s) = pes `AppE` LitE (StringL s)
    toH _ th (Var d) = th `AppE` derefToExp [] d
    go x = do
        let y = mkName $ '_' : x
        return (x, y)
...
defClause = do
    ...
    rm <- [|renderMessage|]
    return $ Clause
        [VarP a, VarP b, ConP (mkName ":") [WildP, VarP c], VarP d]
        (NormalB $ rm `AppE` VarE a `AppE` VarE b `AppE` VarE c `AppE` VarE d)
        []

toCon :: String -> SDef -> Con
toCon dt (SDef c vs _) =
    RecC (mkName c) $ map go vs
  where
